    ...
    def update(self, obs, actions, old_log_probs, returns, advantages):
        ...
        for _ in range(EPOCHS):
            ...
            for start in range(0, len(obs), MINIBATCH_SIZE):
                ...
                if USE_KL_PENALTY:
                    kl = (old_log_probs[idx] - log_probs).mean()
                    policy_loss = -(ratio * advantages[idx]).mean() + self.beta * kl
        ...
        if USE_KL_PENALTY and kl_divs:
            avg_kl = np.mean(kl_divs)
            if avg_kl > TARGET_KL * 1.5:
                self.beta = min(2.0, self.beta * 1.5)
...
def plot_batch(all_rewards, batch_size=2, title="Change in Batch Reward Standard Deviation"):
